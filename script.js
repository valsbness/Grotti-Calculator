...
const full = [
  ...
  ["Otros","Motor V12 Upgrade",250000,500000],
  ["Otros","Motor V8 Upgrade",35000,50000],
...
function update(items,targetId,prefix){
  ...
  [5,10,15].forEach(p=>{
    const discPrefix = prefix === "habitual" ? "h" : "f";
    document.getElementById(discPrefix+p+"total").textContent = money(total);
    document.getElementById(discPrefix+p+"pay").textContent = money(total*(1-p/100));
  });
  save();
}
